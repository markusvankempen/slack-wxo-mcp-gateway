...
#
# Full source lives in the private repo (slack-wxo-mcp-gateway-dev).
...
# Docs-consumer scripts only (no deploy that embeds app assumptions beyond modes)
...
# Hard deny: never stage application source
...
rm -f "$STAGING"/Dockerfile "$STAGING"/requirements.txt \
  ...
  "$STAGING"/ui.html 2>/dev/null || true

# Fail if any Python source leaked
...
if [[ -d "$STAGING/bin" ]]; then
  echo "ERROR: bin/ must not be in public docs repo" >&2
  exit 1
fi
...
Publish documentation and package metadata (application source on request).
...
if ! git remote get-url origin >/dev/null 2>&1; then
  gh repo create "$REPO" \
    --public \
    --description "Docs for Slack↔WxO MCP gateway — source available on request. https://markusvankempen.github.io/" \
